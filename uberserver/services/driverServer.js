...
var source1="";
var destination1="";
var r_id_global = 2152;
var did_global=0;
var distsourceanddest=1;
var sourcetodesttime=1;
var https = require("https");

exports.show_bill = function(msg, callback){
	;
	var res = {};
	var fetchbill = "Select * from nodejsserver.rides where r_id =" + "'" + r_id_global + "'" + ";"; 
...
function handle_requestdriver(msg, callback){
	   ...
	   var res={};
		...
		mysql.fetchData(function(err,loginresults){
			...
			if(loginresults){
			res.code = "200";
			query1="select r_id, r_source, r_destination from rides where d_id="+loginresults[0].did+" and r_status=0";
...
var conn=mysql1.getConnection();
    ...
    + currentdate.getHours()   
    + currentdate.getMinutes()  
    + currentdate.getSeconds()+Math.random();
   	var traveldate=currentdate.getDate() + "/"
    + (currentdate.getMonth()+1)  + "/" 
    + currentdate.getFullYear() ;
   	var traveltime=currentdate.getHours() + ":"  
   ...
   var drivervariable=1;
   	...
   	if(currentdate.getHours >=10 && currentdate.getHours <= 12 || currentdate.getHours >=20 && currentdate.getHours <= 24 )
    {
   	...
   	var query=conn.query("insert into rides values("+billid+",'"+source1+"','"+destination1+"',"+msg.did+","+billid+","+billamount+",0,'"+result2[0].name+"',"+msg.uid+",'"+traveldate+"','"+traveltime+"',2,2,"+distsourceanddest+","+sourcetodesttime+",'"+msg.ufname+"')",function(err,result2){
    ...
    var conn=mysql1.getConnection();
        ...
        exports.updateRideCall = function(msg, callback){
            ...
            var conn=mysql1.getConnection();
                ...
                var query=conn.query("update rides set r_destination='"+d1+"' where d_id='"+did_global+"' and u_id=102",function(err,results){
